...
export async function startApi({ port, host = '127.0.0.1', status, node: nodeRef, source, k, mempool = null, log = () => {} }) {
  ...
  const api = { mempool, template: null, server, broadcast(msg) { const b = new TextEncoder().encode(JSON.stringify(msg)); for (const c of clients) { try { c.send(b); } catch {} } }, clients };
  return api;
}
