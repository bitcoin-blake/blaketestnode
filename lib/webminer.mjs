...
export function makeWebMiner({ k, hash, pow, blake2b, signer, node, mempool = null, key, payScript, chain, url, agent = 'blaketestnode-webminer/0.1', log = () => {}, now = () => Math.floor(Date.now() / 1000) }) {
  ...
  // a nonce the miner found on `job.work`: the full header, the hashes, and the signed share
  function share(job, nonce) {
    const header = { ...job.header, nonce, nonce2: 0, nonce3: 0, timeOffset: 0, extranonce: job.extranonce };
