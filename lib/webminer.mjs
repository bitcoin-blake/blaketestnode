...
// maxSharesPerSecond: manners. A share must be at the assignment's target (SPEC 8.1), so a tab given a low one
// cannot raise it; what it can do is not flood the coordinator while vardiff catches up. Beyond the cap a share
// is dropped and counted; the work it proves is lost to the miner, which is the incentive to let vardiff settle.
export function makeWebMiner({ k, hash, pow, blake2b, signer, node, mempool = null, key, payScript, chain, url, agent = 'blaketestnode-webminer/0.1', log = () => {}, now = () => Math.floor(Date.now() / 1000), maxSharesPerSecond = 10, clock = () => Date.now() }) {
  ...
  const st = { connected: false, pool: null, splits: new Map(), assignment: null, acked: 0, refused: 0, lastAck: null, sent: 0, dropped: 0, ws: null, backoff: 1000, closed: false, blocksFound: 0 };
  const bucket = { at: 0, n: 0 }; let dropNoted = 0;
  ...
  function send(ev, { isBlock = false } = {}) {
    if (!st.connected || !st.ws) return false;
    const t = clock(); if (t - bucket.at >= 1000) { bucket.at = t; bucket.n = 0; }
    if (!isBlock && ++bucket.n > maxSharesPerSecond) { st.dropped++; if (t - dropNoted > 10000) { dropNoted = t; log(`manners: over ${maxSharesPerSecond} shares/s at this difficulty; dropping the excess until the pool raises it (${st.dropped} dropped so far)`); } return false; } // a block is never dropped
    st.ws.send(JSON.stringify({ type: 'share', event: ev })); st.sent++; return true;
  }
  connect();
